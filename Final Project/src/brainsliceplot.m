function brainsliceplot(data,days)
...
% subplot propertiessubPlotFontSize = 10;     % the font size for subplots
...
colorLimits = [0,4.19*10^4];                    % the color bar limits, the dataset contains numbers between 0 and some large number.
...
mainPlot.Title.String = ['Time = ' num2str(days) '.0 days. Brain MRI slices along the Z-direction, Rat W09. No radiation treatment']; % set the title of the figure
...
for irow = nrow:-1:1
    for icol = 1:ncol
        ...
        axes( 'position', [ ... set the position of the axes for each subplot
